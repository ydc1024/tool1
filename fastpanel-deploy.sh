...
# PHP Calculator Platform FastPanel 专用部署脚本
# 适用于 FastPanel 面板环境
# 目标路径: /var/www/besthammer_c_usr/data/www/besthammer.club

set -e  # 遇到错误立即退出

echo "🚀 PHP Calculator Platform FastPanel 部署脚本"
echo "================================================"
...
if [ "$EUID" -ne 0 ]; then
    ...
    echo "使用方法: sudo bash fastpanel-deploy.sh"
    exit 1
fi
...
if [ "$(echo "$PHP_VERSION >= 8.2" | bc -l 2>/dev/null || echo 0)" -eq 0 ]; then
    log_warning "建议使用 PHP 8.2 或更高版本，当前版本: $PHP_VERSION"
fi
