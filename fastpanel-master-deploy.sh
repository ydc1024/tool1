...
# 第一步：基础部署（使用修复版）
log_step "第1步：执行基础部署 (fastpanel-deploy-fixed.sh)"
if [ -f "$SCRIPT_DIR/fastpanel-deploy-fixed.sh" ]; then
    chmod +x "$SCRIPT_DIR/fastpanel-deploy-fixed.sh"
    bash "$SCRIPT_DIR/fastpanel-deploy-fixed.sh"
    log_success "基础部署完成"
elif [ -f "$SCRIPT_DIR/fastpanel-deploy.sh" ]; then
    log_warning "使用原版部署脚本（可能存在composer选项问题）"
    chmod +x "$SCRIPT_DIR/fastpanel-deploy.sh"
    bash "$SCRIPT_DIR/fastpanel-deploy.sh"
    log_success "基础部署完成"
else
    log_error "fastpanel-deploy-fixed.sh 或 fastpanel-deploy.sh 文件不存在"
    exit 1
fi
